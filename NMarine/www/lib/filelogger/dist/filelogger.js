  ...
  .factory('$fileLogger', ['$q', '$window', '$cordovaFile', '$timeout', '$filter',
    ...
    function writeLog(message) {
      ...
      if (isBrowser()) {
        ...
        if (!$window.cordova || !$window.cordova.file || !$window.cordova.file.externalRootDirectory) {
          q.reject('cordova.file.externalRootDirectory is not available');
          return q.promise;
        }

        $cordovaFile.checkFile(cordova.file.externalRootDirectory + 'MarineLogs/', storageFilename).then(
          function() {
            // writeExistingFile(path, fileName, text)
            $cordovaFile.writeExistingFile(cordova.file.externalRootDirectory + 'MarineLogs/', storageFilename, message).then(
              function() {
                q.resolve();
              },
              function(error) {
          ...
          function() {
            ...
            $cordovaFile.writeFile(cordova.file.externalRootDirectory + 'MarineLogs/', storageFilename, message, true).then(
              function() {
                q.resolve();
              },
              function(error) {
    ...
    function getLogfile() {
      ...
      if (isBrowser()) {
        ...
        $cordovaFile.readAsText(cordova.file.externalRootDirectory + 'MarineLogs/', storageFilename).then(
          function(result) {
            q.resolve(result);
          },
          function(error) {
    ...
    function deleteLogfile() {
      ...
      if (isBrowser()) {
        ...
        $cordovaFile.removeFile(cordova.file.externalRootDirectory + 'MarineLogs/', storageFilename).then(
          function(result) {
            q.resolve(result);
          },
          function(error) {
    ...
    function checkFile() {
      ...
      if (isBrowser()) {
        ...
        $cordovaFile.checkFile(cordova.file.externalRootDirectory + 'MarineLogs/', storageFilename).then(function(fileEntry) {
